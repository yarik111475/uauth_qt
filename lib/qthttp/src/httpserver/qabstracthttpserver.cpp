...
{
#if defined(QT_WEBSOCKETS_LIB)
    QObject::connect(&websocketServer_, &QWebSocketServer::newConnection,
...
{
#if QT_CONFIG(ssl)
    QTcpServer *tcpServer = sslEnabled_ ? new QSslServer(sslConfiguration_, this)
                                          : new QTcpServer(this);
#else
...
{
    return websocketServer_.hasPendingConnections();
}
...
{
    return websocketServer_.nextPendingConnection();
}
#endif
