...
/*! \fn template <typename Type> bool HttpRouter::addConverter(const QLatin1String &regexp)
...
/*! \fn template <typename ViewHandler, typename ViewTraits = HttpRouterViewTraits<ViewHandler>> bool HttpRouter::addRule(HttpRouterRule *rule)
...
/*! \fn template<typename ViewHandler, typename ViewTraits = HttpRouterViewTraits<ViewHandler>> auto bindCaptured(ViewHandler &&handler, QRegularExpressionMatch &match) const -> typename ViewTraits::BindableType
...
HttpRouterPrivate::HttpRouterPrivate()
    : converters(defaultConverters)
{}

/*!
    Creates a HttpRouter object with \c defaultConverters.

    \sa defaultConverters()
*/
HttpRouter::HttpRouter()
    : d_ptr(new HttpRouterPrivate)
{}

/*!
    Destroys a HttpRouter.
*/
HttpRouter::~HttpRouter()
{}
...
/*!
    ...
    is replaced with \a regexp.
*/
void HttpRouter::addConverter(const int type, const QLatin1String &regexp)
{
    Q_D(HttpRouter);
...
*/
void HttpRouter::removeConverter(const int type)
{
    Q_D(HttpRouter);
...
/*!
    ...
    \sa defaultConverters()
*/
void HttpRouter::clearConverters()
{
    Q_D(HttpRouter);
    d->converters.clear();
}
...
*/
const QMap<int, QLatin1String> &HttpRouter::converters() const
{
    Q_D(const HttpRouter);
    return d->converters;
}
...
*/
const QMap<int, QLatin1String> &HttpRouter::defaultConverters()
{
    return ::defaultConverters;
}

bool HttpRouter::addRuleImpl(HttpRouterRule *rule,
...
/*!
    ...
    matches the request.
*/
bool HttpRouter::handleRequest(const HttpRequest &request,QAbstractSocket *socket) const
{
    Q_D(const HttpRouter);
