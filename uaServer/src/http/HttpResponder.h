...
{
    Q_DECLARE_PRIVATE(HttpResponder)
...
private:
    HttpResponder(const HttpRequest &request, QAbstractSocket *socket);

    QScopedPointer<HttpResponderPrivate> d_ptr;
};

Q_DECLARE_METATYPE(HttpResponder::StatusCode)
